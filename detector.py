...
class InterferometerList(list):
    ...
    def subtract_signal(self, parameters=None, injection_polarizations=None, waveform_generator=None):
        """ Subtract a signal from noise in each of the three detectors.
    ...
    def plot_data(self, signal=None, outdir='.', label=None):
        ...
        for ii, interferometer in enumerate(self):
            ax = fig.add_subplot(float(len(self)) // len(self), len(self), ii + 1)
...
class Interferometer(object):
    ...
    def subtract_signal(self, parameters=None, injection_polarizations=None,
                      waveform_generator=None):
        """ Subtract a signal into noise
        ...
        if injection_polarizations is None:
            ...
            if injection_polarizations is None:
                raise ValueError(
                    'Trying to subtract signal which is None. The most likely cause'
                    ' is that waveform_generator.frequency_domain_strain returned'
                    ' None. This can be caused if, e.g., mass_2 > mass_1.')
        ...
        if np.shape(self.frequency_domain_strain).__eq__(np.shape(signal_ifo)):
            ...
            logger.info('Subtracting from zero noise.')
            self.set_strain_data_from_frequency_domain_strain(
                signal_ifo,
                sampling_frequency=self.strain_data.sampling_frequency,
                duration=self.strain_data.duration,
